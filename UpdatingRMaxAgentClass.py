...
class UpdatingRMaxAgent(Agent):
    '''
    Implementation for an R-Max Agent [Brafman and Tennenholtz 2003]
    '''

    def __init__(self, actions, gamma=0.95, horizon=4, s_a_threshold=1, num_sample_eps=20, name="UpdatingRMax-h"):
    ...
    def _get_reward(self, state, action):
        ...
        elif self.cur_eps > self.num_sample_eps:
    ...
    def set_init_q_function(self, q_func):
        ...
        Qinit(s, a) <- U(s, a)
        '''
        self.init_q_func = copy.deepcopy(self.q_func)

    def get_q_function(self):
        q_func = defaultdict(lambda: defaultdict(lambda: 0.0))
        for x in self.init_q_func:
            for y in self.init_q_func[x]:
                q_val = self.get_q_value(x, y)
                q_func[x][y] = q_val
        return q_func

    def update_init_q_function(self):
        new_q_func = self.get_q_function()
